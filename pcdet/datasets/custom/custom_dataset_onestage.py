...
from ...utils import box_utils, calibration_kitti, common_utils, object3d_custom, object3d_onestage
...
class CustomDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        idx = int(idx)
        lidar_files = glob.glob('/data/home/scv6a15/CustomData/LidarBin/*.bin')
        lidar_files.sort()
    ...
    def get_radar_feature(self, idx):
        idx = int(idx)
        radar_files = glob.glob('/data/home/scv6a15/CustomData/featurescombine1010/*.npy')
        radar_files.sort()
        radar_feature = np.load(radar_files[idx])
        return radar_feature

    def get_centers(self, idx):
        idx = int(idx)
        centers_files = glob.glob('/data/home/scv6a15/CustomData/centers/*.npy')
        centers_files.sort()
        centers = np.load(centers_files[idx])
        return centers
    
    def get_label(self, idx):
        idx = int(idx)
        label_files = glob.glob('/data/home/scv6a15/CustomData/Label(KittiFormal)2/*.txt')
        label_files.sort()
        return object3d_custom.get_objects_from_label(label_files[idx])
    
    def get_onestage_boxes(self, idx):
        idx = int(idx)
        boxes_files = glob.glob(r'/data/home/scv6a15/CustomData/data/*.txt')
        boxes_files.sort()
        return object3d_onestage.get_objects_from_label(boxes_files[idx])
    ...
    def get_infos(self, num_workers = 4, sample_id_list = None):
        ...
        def process_single_scene(sample_idx):
            ...
            boxes_list = self.get_onestage_boxes(sample_idx)
            annotations['roi_labels'] = np.array([boxes.cls_id for boxes in boxes_list])
            annotations['roi_dimensions'] = np.array([[boxes.l, boxes.w, boxes.h] for boxes in boxes_list])
            annotations['roi_location'] = np.concatenate([boxes.loc.reshape(1, 3) for boxes in boxes_list], axis=0)
            annotations['roi_rotation_y'] = np.array([boxes.ry for boxes in boxes_list])
            annotations['roi_scores'] = np.array([boxes.score for boxes in boxes_list])
    ...
    def __getitem__(self, index):
        ...
        centers = self.get_centers(sample_idx)
        ...
        if 'annos' in info:
            ...
            roi_loc, roi_dims, roi_rots = annos['roi_location'], annos['roi_dimensions'], annos['roi_rotation_y']
            rois = np.concatenate([roi_loc,roi_dims, roi_rots[..., np.newaxis]], axis = 1).astype(np.float32)
            roi_labels = annos['roi_labels']
            roi_scores = annos['roi_scores']
